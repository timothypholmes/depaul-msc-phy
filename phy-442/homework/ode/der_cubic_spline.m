function der_cubic_spline(x, y, spacing)
%% Tridiagonal Program
%   - Curve fit
%   
%   Input Arg
...
r = [0; r; 0;];
r(1) =  (6 * (y(2) - y(1)))/h(1) - p_prime(1); 
r(n) =  (-6 * (y(n) - y(n - 1)))/h(n - 1) - p_prime(1);
A(1, 2) = h(1);
A(1, 1) = 2 * h(1);
A(n - 1, n) = h(n - 1);
A(n, n - 1) = h(n - 1);
...
function [x, y, p_prime] = homework_1_values()
