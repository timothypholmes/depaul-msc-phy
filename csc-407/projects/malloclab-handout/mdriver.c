...
/* The filenames of the bonus tracefiles */
static char *bonus_tracefiles[] = {
  BONUS_TRACEFILES, NULL
};
...
int main(int argc, char **argv)
{
  ...
  int bonus_traces = 0;  /* whether we use bonus traces */
   ...
   */
  while ((c = getopt(argc, argv, "f:t:hvVgbl")) != EOF) {
    switch (c) {
      case 'b': /* Use bonus traces. */
        bonus_traces = 1;
        break;
  ...
  if (tracefiles == NULL) {
    if (bonus_traces) {
      tracefiles = bonus_tracefiles;
      num_tracefiles = sizeof(bonus_tracefiles) / sizeof(char *) - 1;
    }
    else {
  ...
  if (errors == 0) {
    ...
    p1 = UTIL_WEIGHT * (avg_mm_util - 0.3f) / (0.9f - 0.3f) ;
    if (p1 < 0.f)
      p1 = 0.0;
  ...
  if (autograder) {
    ...
    double corpts = (double) numcorrect / (double) num_tracefiles * cormaxpts;
    double perfpts = perfindex / 95.f * perfmaxpts;

    printf("correct:%d/%d (= %.1f/%.0f)\n", numcorrect, num_tracefiles,
           corpts, cormaxpts);
    printf("perfidx:%.1f/%d (= %.1f/%.0f)\n", perfindex, 100, perfpts, perfmaxpts);
    if (bonus_traces)
      printf ("bonus ");
    printf("points:%.1f/%.0f\n", corpts + perfpts, cormaxpts + perfmaxpts);
  }
...
static void usage(void)
{
  fprintf(stderr, "Usage: mdriver [-hvVbl] [-f <file>] [-t <dir>]\n");
  fprintf(stderr, "Options\n");
  fprintf(stderr, "\t-b         Use the bonus trace files.\n");
